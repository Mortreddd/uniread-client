...
const inputVariant = cva(
  "inline-flex outline-0 justify-start items-center transition-colors rounded font-medium duration-200 ease-in-out placeholder:text-gray-500",
  {
    variants: {
      variant: {
        primary:
          "text-primary ring-primary ring-0 border-2 border-primary focus focus:ring-2 focus:border-0 focus:ring-primary",
        default:
...
export const Input = memo(forwardRef<HTMLInputElement, InputProps>((({className, variant, ...props}, ref) => {
    return(
        <input
      ref={ref}
      {...props}
