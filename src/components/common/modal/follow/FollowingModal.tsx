...
function FollowingModal({ authorId }: FollowModalProps, ref: Ref<ModalRef>) {
    ...
    const [selectedUserFollowings, setSelectedUserFollowings] = useState<Follow[]>([]);
    ...
    useEffect(() => {
        ...
        setSelectedUserFollowings(data.content.filter((f) => f.following.id !== user?.id));
    }, [data, user?.followers]);
            ...
            <div className="min-w-96 w-fit h-fit">
                ...
                <div className="flex flex-col items-start w-full divide-y-2 overflow-y-auto">
                    <div className="bg-gray-100 rounded px-4 py-2 w-full">
                        ...
                        ) : !loading && selectedUserFollowings.length > 0 ? (
                            selectedUserFollowings.map((follow, index) => (
                                <AuthorFollowInfo
                                    key={index}
                                    follow={follow}
                                    isMutualFollowing={isMutualFollowing(follow.following.id)}
                                    onFollow={(() => onFollow(follow.following.id))}
                                    onUnfollow={(() => onUnfollow(follow.following.id))}
                                />
                            ))
                        ) : (
