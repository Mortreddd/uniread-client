...
interface CreateTagProps extends Omit<
  TagDetail,
  "id" | "usageCount" | "updatedAt" | "createdAt"
> {}

function AddTagModal({ onCreate }: AddTagModalProps, ref: Ref<ModalRef>) {
  ...
  const onSubmit: SubmitHandler<CreateTagProps> = async (data) => {
    try {
      ...
      if (response?.fieldErrors) {
        Object.entries(response.fieldErrors).forEach(([field, message]) => {
          setError(field as keyof CreateTagProps, {
            type: "server",
      ...
      >
        ...
        <div className="flex items-center w-full gap-2 md:gap-4">
          ...
          <Button
            ...
            disabled={createTagMutation.isPending || !isValid}
          >
