...
import { BookDetail, Genre, CreateBookFormProps } from "@/features/books/types/Book.ts";
...
import Toggle from "@/shared/components/form/Toggle.tsx";
import { ErrorResponse } from "@/types/Error.ts";

interface AddBookModalProps {}

function AddBookModal({}: AddBookModalProps, ref: Ref<ModalRef>) {
  const { showToast } = useToast();
  const navigate = useNavigate();
  const { data } = useGetGenres();
  const [{ loading }, setRequestState] = useState<RequestState<BookDetail>>({
    data: null,
    error: null,
    loading: false,
  });
  ...
  async function onSubmit(event: FormEvent) {
    ...
    await api
      ...
      .then((response: AxiosResponse<BookDetail>) => {
        ...
        setSelectedGenres([]);
        navigate(`/workspace/stories/${response.data.id}`);
      })
...
export default forwardRef(AddBookModal);
